...
if __name__ == '__main__':
    ...
    parser.add_argument('--merge_step', default=None, help='step period \
                        ...
                        type=int)
    parser.add_argument('--minimum_epsilon', default=None,
                        help='final epsilon value', type=float)
    parser.add_argument('--env', default='reticolo', help='set environment')
    parser.add_argument('--broadband', default=False,
                        help='set broadband input')
    ...
    parser.add_argument('--save_model', default=True,
                        help='decide wheter to save model weights or not')
    ...
    parser.add_argument('--tb', default=True,
                        help='tensorboard setting, True/False')

    # decide wheter to save device images or not
    parser.add_argument('--save_devices', default=False,
                        help='decide whether to save device images or not - jpg')

    # decide wheter to save np structure or not
    parser.add_argument('--save_np_struct', default=False,
                        help='decide wheter to save numpy structure of devices or not')

    # decide wheter to save summary or not
    parser.add_argument('--save_summary', default=True,
                        help='decide wheter to save the model or not')

    # decide wheter to save source code or not
    parser.add_argument('--source_code_save', default=True,
                        help='decide wheter to save the source code or not')

    parser.add_argument('--network', default='DQN',
                        help='decide which network to use: "DQN"(default), "Double"(Double DQN), "Dueling"(Dueling DQN)')

    parser.add_argument('--optimizer', default='Adam',
                        help='decide which optimizer to use: Adam, AdamW, SGD, RMSprop')

    parser.add_argument('--save_optimum', default=True,
                        help='decide whether to save the optimal structure or not')

    parser.add_argument('--tag', default='',
                        help='folder tag name for experiments')

    args = parser.parse_args()

    path_json = '../UNet/config/config.json'
    ...
    timeFolderName = t.strftime("%Y_%m_%d_%H_%M_%S") + "/wl" + str(args.wl) + \
                     "_angle" + str(args.ang) + "_ncells" + str(
        int(args.ncells))
    ...
    if args.env == 'reticolo':
        ...
        env = ReticoloDeflector(int(args.nG), int(args.ncells), args.wl,
                                args.ang)
        env_val = ReticoloDeflector(int(args.nG), int(args.ncells), args.wl,
                                    args.ang)

    if args.network == 'DQN' or args.network == 'Double':
        q = Qnet(int(args.ncells))
        q_target = Qnet(int(args.ncells))
        q_target.load_state_dict(q.state_dict())

    elif args.network == 'Dueling':
        q = DuelingQnet(int(args.ncells))
        q_target = DuelingQnet(int(args.ncells))
        q_target.load_state_dict(q.state_dict())
        ...
        q.load_state_dict(torch.load(
            ...
            map_location=device))
        q_target.load_state_dict(torch.load(
            'July5_256_7man_1100_70_0.00150431_0.00243219_stateDict.pt',
            map_location=device))
    ...
    if args.source_code_save == True:
        ...
        shutil.copy(os.getcwd() + '/logger.py',
                    filepath + path_logs + 'logger.py')
        shutil.copy(os.getcwd() + '/network.py',
                    filepath + path_logs + 'network.py')
        shutil.copy(os.getcwd() + '/deflector_reticolo.py',
                    filepath + path_logs + 'deflector_reticolo.py')
        shutil.copy(os.getcwd() + '/replaybuffer.py',
                    filepath + path_logs + 'replaybuffer.py')
    ...
    while True:
        ...
        for t in range(int(args.epilen)):
            ...
            epsilon = max(args.minimum_epsilon,
                          0.9 * (1. - count / args.eps_greedy_period))
            q.eval()
            ...
            if args.save_optimum == True:
                if eff_next > eff_flag:
                    ...
                    plt.yticks([])
                    plt.savefig(filepath + path_devices_max + 'max.png',
                                format='png')
                    plt.savefig(filepath + path_devices_max + 'max.eps',
                                format='eps')
                    np.save(filepath + path_np_struct_max + 'max_structure.npy',
                            s)
                    np.save(
                        filepath + path_np_struct_max + 'max_efficiency.npy',
                        np.array(eff_flag))
                    np.save(
                        filepath + path_np_struct_max + 'max_stepnumber.npy',
                        np.array(count))
                    plt.close()
        ...
        if n_epi % int(args.printint) == 0 and n_epi != 0:
            ...
            if epi_length != 0:
                one_step_average_reward = np.append(one_step_average_reward,
            ...
            eff_val_max_zero_np = np.append(eff_val_max_zero_np,
                                            eff_val_max_zero)
            epi_len_val_zero_np = np.append(epi_len_val_zero_np,
                                            epi_len_val_zero)

            np.save(filepath + path_logs + 'x_step.npy', x_step)
            np.save(filepath + path_logs + 'x_episode.npy', x_episode)
            np.save(filepath + path_logs + 'one_step_average_reward.npy',
                    one_step_average_reward)
            np.save(filepath + path_logs + 'final_step_efficiency.npy',
                    final_step_efficiency)
            ...
            np.save(filepath + path_logs + 'eff_val_max_zero.npy',
                    eff_val_max_zero_np)
            np.save(filepath + path_logs + 'epi_len_val_zero.npy',
                    epi_len_val_zero_np)
            ...
            if args.tb == True:
                ...
                writer.add_scalar('mean of max validation / step', eff_val_mean,
                                  count)
                writer.add_scalar('std of max validation / step', eff_val_std,
                                  count)
                writer.add_scalar('max of max validation / step', eff_val_max,
                                  count)
                writer.add_scalar('max of validation zero / step',
                                  eff_val_max_zero, count)
                writer.add_scalar('episode length val zero / step',
                                  epi_len_val_zero, count)
                if (memory.size() > int(args.train_start_memory_size)
                        and count % int(args.train_step) == 0):
            ...
            log_utils.write_logs(loggername, lgr, sh, n_epi, eff_val_max_zero, \
                              np.max(eff_epi_st), eff_flag, epi_length,
                              memory.size(), epsilon * 100, count)
            log_utils.write_json_hyperparameter(filepath + path_logs, args)

            if args.save_devices == True:
                log_utils.plot_device(filepath + path_devices, s, n_epi)

            if args.save_np_struct == True:
                log_utils.plot_struct(filepath + path_np_struct, s, n_epi)

            if args.checkpoint == True:
                torch.save(q.state_dict(),
                           filepath + path_model + str(count) + 'steps_q')
                torch.save(q_target.state_dict(), filepath + path_model + str(
                    count) + 'steps_q_target')
            else:
                torch.save(q.state_dict(), filepath + path_model + 'q')
                torch.save(q_target.state_dict(),
                           filepath + path_model + 'q_target')

        n_epi += 1

    if args.save_summary == True:
        log_utils.plot_summary(q, epi_len_st, s, filepath + path_summary)
    ...
    if args.save_model == True:
        ...
        for param_tensor in q_target.state_dict():
            print(param_tensor, "\t",
                  q_target.state_dict()[param_tensor].size())
        ...
        torch.save(q_target.state_dict(),
                   filepath + path_model + 'final_steps_q_target')

    env.close()
    env_val.close()

    print('max efficiency: ', eff_flag)
    print('max stepnumber: ',
          np.load(filepath + path_np_struct_max + 'max_stepnumber.npy'))
    print('max strucutre: ',
          np.load(filepath + path_np_struct_max + 'max_structure.npy'))
