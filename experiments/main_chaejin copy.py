...
if __name__== '__main__':
    ...
    parser.add_argument('--network', default='ShallowUQnet', help='decide which network to use: "DQN"(default), "Double"(Double DQN), "Dueling"(Dueling DQN)')
    ...
    while(True):
        ...
        for t in range(int(args.epilen)):
            ...
            if (memory.size() > int(args.train_start_memory_size)
                ...
                q.train()
                loss = pirl.train.train_network(q, q_target, memory, optimizer, int(args.train_num), \
                                                int(args.batch_size), args.gamma, double=double)

                if count % int(args.merge_step) == 0:

                    pirl.train.merge_network_weights(q_target.named_parameters(),
        ...
        if n_epi % int(args.printint) == 0 and n_epi != 0:
            ...
            if args.save_devices == True:
                logger.plot_device(filepath + path_devices, s, n_epi)
            
            if args.save_np_struct == True: 
                logger.plot_struct(filepath + path_np_struct, s, n_epi)
    ...
    if args.save_summary == True:
        logger.plot_summary(q, epi_len_st, s, filepath + path_summary)
