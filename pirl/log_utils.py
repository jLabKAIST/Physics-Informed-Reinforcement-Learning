...
def write_logs(loggername, lgr, sh, n_epi, eff, effmax_episode, effmax_overall, \
    ...
    lgr.info(
        'n_epi: %s, eff: %s, effmax_episode: %s, effmax_overall: %s, episode_length: %s, n_buffer: %s, epsilon_percent:%s, count:%s', \
        n_epi, eff, effmax_episode, effmax_overall, episode_length, n_buffer,
        epsilon_percent, count)
    ...
    if os.path.isfile(loggername + '.csv'):
        with open(loggername + '.csv', 'a', newline='') as f:
            w = csv.writer(f)
            w.writerow(
                [time.strftime('%Y_%m_%d %H:%M:%S'), n_epi, eff, effmax_episode,
                 effmax_overall, episode_length, n_buffer, epsilon_percent,
                 count])

    else:
        output = csv.writer(open(loggername + '.csv', 'w', newline=''))
        output.writerow(
            ['time', 'n_epi', 'eff', 'effmax_episode', 'effmax_overall',
             'episode_length', 'n_buffer', 'epsilon [%]', 'count'])
        output.writerow(
            [time.strftime('%Y_%m_%d %H:%M:%S'), n_epi, eff, effmax_episode,
             effmax_overall, episode_length, n_buffer, epsilon_percent, count])
...
def plot_summary(q, epi_len_st, struct, path_summary):
    plt.figure()
    plt.plot(q.effdata)
    plt.title('eff')
...
def plot_device(path_devices, struct, n_epi):
...
def plot_struct(path_np_struct, struct, n_epi):
