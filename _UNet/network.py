...
class convrelu(nn.Module):
    def __init__(self, nin, nout):
        ...
        self.convrelu = nn.Sequential(
            nn.Conv1d(
                nin, nout, 3, padding='same', padding_mode='circular'),
            nn.BatchNorm1d(nout),
...
def train_network(q, q_target, memory, optimizer, train_number, batch_size,
    ...
    for i in range(int(train_number)):
        ...
        s, a, r, s_prime, done_mask = s.to(device), a.to(device), r.to(
            device), s_prime.to(device), done_mask.to(device)  # ml2
        q_out = q(s)
        ...
        target = done_mask * (r + gamma * max_q_prime) + (1 - done_mask) * 1 / (
                    1 - gamma) * r
