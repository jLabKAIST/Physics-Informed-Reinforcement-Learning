...
if __name__== '__main__':
    ...
    parser.add_argument('--network', default='UQnet_notrain', help='decide which network to use: "DQN"(default), "Double"(Double DQN), "Dueling"(Dueling DQN)')
    ...
    path_json = f'{CURR_DIR}/UNet/config/config.json' ##Chaejin
    ...
    print("os.listdir", os.listdir()) ##Chaejin
    ...
    filepath = 'UNet/experiments/'+args.network+'/'+args.tag+'/'+timeFolderName
    ...
    if args.network=='DQN' or args.network=='Double':
        ...
        #q_target.load_state_dict(q.state_dict()) 8/30
        q.load_state_dict(torch.load(f'{CURR_DIR}/UNet/Aug29_fcn_256_2man_1100_70_0.00619343_0.00632467_stateDict.pt',map_location=device)) ## 8/30
        q_target.load_state_dict(torch.load('{CURR_DIR}./UNet/Aug29_fcn_256_2man_1100_70_0.00619343_0.00632467_stateDict.pt',map_location=device)) ## 8/30
    ...
    print('os.getcwd()', os.getcwd()) ##Chaejin
    ...
    while(True):
        ...
        if n_epi % int(args.printint) == 0 and n_epi != 0:
            ...
            for i in range(int(args.val_num)):
                ...
                for t in range(int(args.epilen)):
                    q.eval()
                    a = q.sample_action(torch.from_numpy(s).float().to(device), epsilon_val)
            ...
            for t in range(int(args.epilen)):
                q.eval()
                a = q.sample_action(torch.from_numpy(s).float().to(device), epsilon_val_zero)
            ...
            if (memory.size() > int(args.train_start_memory_size)
                and count % int(args.train_step) == 0):
                loss_numpy = loss.detach().cpu().numpy() #####cj
            ...
            # ml2 : 매번 세이브 하면 오버헤드가 클수 있어서, 분석할 시점에만 저장하면 좋을것 같음
